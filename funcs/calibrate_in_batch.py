...
def calibrate_all_subjects(text_data_list, model_index, reading_data_list, calibration_data_list, calibrate_mode, subject_start, subject_end, max_iteration=100, distance_threshold=64):
    text_data = text_data_list[model_index]

    for subject_index in range(subject_start, subject_end):
...
def calibrate_in_batch(calibrate_mode, model_indices, subject_start, subject_end, validation_num=25, random_seed=0):
    """
...
def calibrate_in_batch_for_different_model(calibrate_mode, model_index, subject_start, subject_end, validation_num=25, random_seed=0):
    np.random.seed(random_seed)
    ...
    # 这里改成一次只能处理1个model index，下面代码都注释了。
    # for model_index in range(len(text_data_list)):
    #     # if model_index == 0:
