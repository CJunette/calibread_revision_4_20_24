...
from evaluate_result.evaluate_final_transform_matrix import compute_accuracy_error_for_all_subjects, evaluate_accuracy_error_for_centroid_alignment, compute_accuracy_error_for_all_subjects_given_range
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        arg1 = sys.argv[1]
        arg2 = sys.argv[2]
        configs.subject_start = arg1
        configs.subject_end = arg2
    ...
    calibrate_in_batch("simple_linear_weight", [1]) # 一般情况下校准，使用model_index=1，保证simple_linear_model中的所有参数都被使用。
    # calibrate_in_batch_for_different_training_num("simple_linear_weight", 148, 25, 26, 0, 1)

    # calibrate_in_batch("simple_linear_weight", [0], 9) # 不带simple_linear_model的校准，使用model_index=0，保证simple_linear_model中的所有参数都不被使用。

    # visualize_cali_grad_process(198, 1, 9)
    # visualize_cali_grad_process(55, 1, 9)
    # visualize_all_subject_cali_grad_process(195, 1)
    ...
    # compute_accuracy_error_for_all_subjects(206, 1)
    # compute_accuracy_error_for_all_subjects_given_range(207, 208, [1]*2)
